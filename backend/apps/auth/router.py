...
async def force_migrate(db: AsyncSession = Depends(get_db)):
    ...
    # 0012 — Togo payment operators (correct enum names from migration file)
    await run("0012_payment_operator_tmoney", """
        ALTER TYPE payment_operator_enum ADD VALUE IF NOT EXISTS 'tmoney'
    """)
    await run("0012_payment_operator_flooz", """
        ALTER TYPE payment_operator_enum ADD VALUE IF NOT EXISTS 'flooz'
    """)
    await run("0012_payment_operator_manual", """
        ALTER TYPE payment_operator_enum ADD VALUE IF NOT EXISTS 'manual'
    """)
    await run("0012_payment_method_manual", """
        ALTER TYPE payment_method_enum ADD VALUE IF NOT EXISTS 'manual'
    """)
    ...
    """)
    await run("0015_orders_promo_cols", """
        ALTER TABLE orders
    ...
    # Stamp alembic_version to 0018
    await run("stamp_alembic_delete", "DELETE FROM alembic_version")
    await run("stamp_alembic_insert", """
        INSERT INTO alembic_version (version_num) VALUES ('0018_fix_missing_columns')
    """)

    await db.commit()

    failed = [s for s in steps if s["status"] == "skipped"]
    return {
